...
export default function CompanyDetails() {
  ...
  const review = data.find((item) => item.id === parseInt(id));
  ...
  const handleSortChange = (event) => {
    ...
    // Apply your sorting logic based on the selected option
            ...
            <div className="flex md:flex-row flex-col md:justify-between items-center gap-3">
              <div className=" flex gap-5">
                ...
                <div className="flex flex-col gap-1">
                  <h1 className="text-[24px] font-[600]">{review.title}</h1>
                  <p>{review.desc}</p>
                  <div className="flex items-center gap-2  text-[10px] text-[#777] md:text-[16px] ">
                    ...
                    <p className="text-primary-text-color">{review.rating}</p>
